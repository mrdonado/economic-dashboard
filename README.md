...
Run `npm run fetch:history` to download plot-ready history into `data/history/`, with one JSON file per tracked indicator. Existing observations are preserved: only missing time buckets are requested and periods with no source value are remembered.
